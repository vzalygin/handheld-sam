...
void Render() {
    ...
    // phoneme.
    //
    // Here are two examples. First, For example, consider the word "SUN" (S AH
    // N)
    //
    ...
    //    ------------------------------------------------
    // S
    //    241     0     6     0    73     0    99    61   Use S (weight 18) for
    //    transition instead of AH (weight 2) 241     0     6     0    73     0
    //    99    61   <-- (OutBlendFrames-1) = (1-1) = 0 frames
    // AH
    //      0     2    10     2    66     0    96    59 * <-- InBlendFrames = 3
    //      frames 0     4    14     3    59     0    93    57 * 0     8    18
    //      5    52     0    90    55 * 0    15    22     9    44     1    87 53
    //      0    15    22     9    44     1    87    53
    //      0    15    22     9    44     1    87    53   Use N (weight 8) for
    //      transition instead of AH (weight 2). 0    15    22     9    44     1
    //      87    53   Since N is second phoneme, reverse the IN and OUT values.
    //      0    11    17     8    47     1    98    56 * <-- (InBlendFrames-1)
    //      = (2-1) = 1 frames
    // N
    ...
    //     0     5     6     5    54     0   121    61
    //     0     5     6     5    54     0   121    61
    //     0     5     6     5    54     0   121    61   Use N (weight 8) for
    //     transition instead of AH (weight 2) 0     5     6     5    54     0
    //     121    61   <-- (OutBlendFrames-1) = (1-1) = 0 frames
    // AH
    //     0     8    11     6    51     0   110    59 * <-- InBlendFrames = 2
    //     0    11    16     8    48     0    99    56 *
    //     0    15    22     9    44     1    87    53   Use S (weight 18) for
    //     transition instead of AH (weight 2) 0    15    22     9    44     1
    //     87    53   Since S is second phoneme, reverse the IN and OUT values.
    //     0     9    18     5    51     1    90    55 * <-- (InBlendFrames-1) =
    //     (3-1) = 2 0     4    14     3    58     1    93    57 *
    // S
    ...
    while (1)  // while No. 1
    {
        ...
        // printf("A=phoneme X: %hhu, Y: %hhu, A: %hhu, mem44 %hhu, mem47 %hhu,
        // speedcounter %hhu, mem49: %hhu\n", X, Y, A, mem44, mem47,
        // speedcounter, mem49);
        X++;
        ...
        // printf("A=PGM X: %hhu, Y: %hhu, A: %hhu, mem44 %hhu, mem47 %hhu,
        // speedcounter %hhu, mem49: %hhu\n", X, Y, A, mem44, mem47,
        // speedcounter, mem49);
        ...
        // printf("A=mem49 X: %hhu, Y: %hhu, A: %hhu, mem44 %hhu, mem47 %hhu,
        // speedcounter %hhu, mem49: %hhu\n", X, Y, A, mem44, mem47,
        // speedcounter, mem49);
        mem49 = A;       // mem49 now holds length + position
        A = A + phase2;  // Maybe Problem because of carry flag
        // printf("A=A X: %hhu, Y: %hhu, A: %hhu, mem44 %hhu, mem47 %hhu,
        // speedcounter %hhu, mem49: %hhu\n", X, Y, A, mem44, mem47,
        // speedcounter, mem49);
        // 47776: ADC 42
        speedcounter = A;
        ...
        // printf("A=phase X: %hhu, Y: %hhu, A: %hhu, mem44 %hhu, mem47 %hhu,
        // speedcounter %hhu, mem49: %hhu\n", X, Y, A, mem44, mem47,
        // speedcounter, mem49);
        mem38 = A;
        ...
        if ((X & 128) == 0)
            do  // while No. 2
            {
                ...
                if (mem47 == 168)  // pitch
                {
                    ...
                    A = Read(mem47, mem37);  // value at center of next phoneme
                                             // - end interpolation value
                    // printf("A=ReadIf X: %hhu, Y: %hhu, A: %hhu, mem44 %hhu,
                    // mem47 %hhu, speedcounter %hhu, mem49: %hhu\n", X, Y, A,
                    // mem44, mem47, speedcounter, mem49); A = mem[address];

                    Y = mem36;  // start index of interpolation
                    mem53 =
                        A - Read(mem47,
                                 mem36);  // value to center of current phoneme
                } else {
                    // value to interpolate to
                    A = Read(mem47, speedcounter);
                    // printf("A=ReadElse X: %hhu, Y: %hhu, A: %hhu, mem44 %hhu,
                    // mem47 %hhu, speedcounter %hhu, mem49: %hhu\n", X, Y, A,
                    // mem44, mem47, speedcounter, mem49);
                ...
                while (1)  // while No. 3
                {
                    ...
                    // printf("A=Read2 X: %hhu, Y: %hhu, A: %hhu, mem44 %hhu,
                    // mem47 %hhu, speedcounter %hhu, mem49: %hhu\n", X, Y, A,
                    // mem44, mem47, speedcounter, mem49);
    ...
    while (1) {
        ...
        if (A != 0) {
            ...
            unsigned int p1 =
                phase1 *
                256;  // Fixed point integers because we need to divide later on
            unsigned int p2 = phase2 * 256;
            unsigned int p3 = phase3 * 256;
            int k;
            for (k = 0; k < 5; k++) {
                signed char sp1 =
                    (signed char)pgm_read_byte(&sinus[0xff & (p1 >> 8)]);
                signed char sp2 =
                    (signed char)pgm_read_byte(&sinus[0xff & (p2 >> 8)]);
                signed char rp3 =
                    (signed char)pgm_read_byte(&rectangle[0xff & (p3 >> 8)]);
                ...
                ary[k] = mux;
                p1 += frequency1[Y] * 256 /
                      4;  // Compromise, this becomes a shift and works well
