...
#define _PRINTF_IMPL_0(fmt) do { \
    static const char MSG_IMPL_0[] PROGMEM = fmt; \
    printf(copy_pgm_str(MSG_IMPL_0)); \
} while(0)
#define _PRINTF_IMPL_1(fmt, ...) do { \
    static const char MSG_IMPL_1[] PROGMEM = fmt; \
    printf(copy_pgm_str(MSG_IMPL_1), __VA_ARGS__); \
} while(0)

extern char printf_buffer[256] NOINITMEM;
